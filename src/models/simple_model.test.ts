...
test("concurrent changes w/ server", () => {
  ...
  // Server change saves
  u._save(true);
  // Maybe Save can check if the change is already applied in indexDB. If it is
  // then we don't need to send it to the server since we received that change?
  //
  // E.g. oh, the name is already "pbardea". Maybe original reads from indexDB at
  // the start of save?
  //
